...
RUN apt update && \
    ...
    apt install -y ninja-build build-essential cmake tree curl dkms rename unzip && \
    ...
    apt install -y libgtk2.0-dev libtbb-dev libswscale-dev libavdevice-dev libjpeg-dev libpng-dev && \
...
RUN wget https://github.com/llvm/llvm-project/releases/download/llvmorg-16.0.4/clang+llvm-16.0.4-x86_64-linux-gnu-ubuntu-22.04.tar.xz && \
    ...
    sudo mv clang+llvm-16.0.4-x86_64-linux-gnu-ubuntu-22.04/* /usr && \
